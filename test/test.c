...
void node_dummy_config(mdl_node_ctx_t *node_ctx);
...
void test_mdl_cobs_encode_decode_ok(void)
{
    ...
    TEST_ASSERT_TRUE(mdl_payload_cobs_encode(encoded, &encoded_len, payload_in, payload_in_len));
    TEST_ASSERT_TRUE(mdl_payload_cobs_decode(payload_out, &payload_out_len, encoded, encoded_len));
...
void test_mdl_cobs_encode_nok(void)
{
    srand(0xdeadbeef); // Seed for reproducibility
    ...
    /* Invalid arguments. */
    TEST_ASSERT_FALSE(mdl_payload_cobs_encode(NULL, &encoded_len, payload_in, payload_in_len));
    TEST_ASSERT_FALSE(mdl_payload_cobs_encode(encoded, NULL, payload_in, payload_in_len));
    TEST_ASSERT_FALSE(mdl_payload_cobs_encode(encoded, &encoded_len, NULL, payload_in_len));

    /* Not enough space to encode message. */
    payload_in_len = 100;
    encoded_len = 10; /* Not enough space. */
    TEST_ASSERT_FALSE(mdl_payload_cobs_encode(encoded, &encoded_len, payload_in, payload_in_len));
...
void test_mdl_cobs_decode_nok(void)
{
    srand(0xdeadbeef); // Seed for reproducibility
    ...
    /* Invalid arguments. */
    TEST_ASSERT_FALSE(mdl_payload_cobs_decode(NULL, &payload_out_len, encoded, encoded_len));
    TEST_ASSERT_FALSE(mdl_payload_cobs_decode(payload_out, NULL, encoded, encoded_len));
    TEST_ASSERT_FALSE(mdl_payload_cobs_decode(payload_out, &payload_out_len, NULL, encoded_len));
    TEST_ASSERT_FALSE(mdl_payload_cobs_decode(payload_out, &payload_out_len, NULL, 0));

    /* Encoding not 0 terminated. */
    payload_out_len = sizeof(payload_out); /* "unlimited" */
    encoded_len = 4;
    memcpy(encoded, (uint8_t[]){3, 1, 2, 3}, encoded_len); /* Encoded version of {1,2,3} without trailing 0 */
    TEST_ASSERT_FALSE(mdl_payload_cobs_decode(payload_out, &payload_out_len, encoded, encoded_len));
...
void test_mdl_checksum_calculate(void)
{
    srand(0xdeadbeef); // Seed for reproducibility
    uint8_t checksum = 0;
    uint8_t data[256] = {0};
    ...
    for (int i = 0; i < 25; i++)
    {
        ...
        checksum = mdl_checksum_calculate(data, len);
        TEST_ASSERT_NOT_EQUAL(MDL_CHECKSUM_OK, checksum); /* Random data should not give valid checksum. */
        data[len] = (uint8_t)(checksum);                  /* Add checksum byte. */
        checksum = mdl_checksum_calculate(data, len + 1);
        TEST_ASSERT_EQUAL(MDL_CHECKSUM_OK, checksum); /* Data should be good now. */
    }

    /* Test invalid arg cases: */
    TEST_ASSERT_EQUAL(MDL_CHECKSUM_OK, mdl_checksum_calculate(NULL, 0));
    TEST_ASSERT_EQUAL(MDL_CHECKSUM_OK, mdl_checksum_calculate(data, 0));
}

//----------------------------------------------------------------------------------------------------------------------

void test_mdl_parity_check(void)
{
    mdl_msg_header_t header = {0};

    /* Parity Ok */
    memcpy(header.raw, (uint8_t[3]){0b00000000, 0b00000000, 0b00000000}, sizeof(header.raw));
    TEST_ASSERT_TRUE(mdl_header_parity_check(header)); // All zeros -> even parity

    memcpy(header.raw, (uint8_t[3]){0b11111111, 0b11111111, 0b11111111}, sizeof(header.raw));
    TEST_ASSERT_TRUE(mdl_header_parity_check(header)); // All ones -> even parity

    memcpy(header.raw, (uint8_t[3]){0b01010001, 0b00011000, 0b10000000}, sizeof(header.raw));
    TEST_ASSERT_TRUE(mdl_header_parity_check(header)); // Mixed bits -> even parity

    /* Parity Error */
    memcpy(header.raw, (uint8_t[3]){0b00000000, 0b00000000, 0b00000001}, sizeof(header.raw));
    TEST_ASSERT_FALSE(mdl_header_parity_check(header)); // One bit set -> odd parity

    memcpy(header.raw, (uint8_t[3]){0b11111111, 0b11111111, 0b11111110}, sizeof(header.raw));
    TEST_ASSERT_FALSE(mdl_header_parity_check(header)); // One bit cleared -> odd parity

    memcpy(header.raw, (uint8_t[3]){0b01100001, 0b00011000, 0b11000000}, sizeof(header.raw));
    TEST_ASSERT_FALSE(mdl_header_parity_check(header)); // Mixed bits -> odd parity
}

//--------------------------------------------------------------------------------------------------------------------------------------

void test_mdl_header_action_set_get(void)
{
    mdl_msg_header_t header = {0};

    mdl_header_action_set(&header, MDL_ACTION_READ);
    TEST_ASSERT_EQUAL(0b00000000, header.raw[0]);
    TEST_ASSERT_EQUAL(0, header.raw[1]);
    TEST_ASSERT_EQUAL(0, header.raw[2]);
    TEST_ASSERT_EQUAL(MDL_ACTION_READ, mdl_header_action_get(header));

    mdl_header_action_set(&header, MDL_ACTION_WRITE);
    TEST_ASSERT_EQUAL(0b01000000, header.raw[0]);
    TEST_ASSERT_EQUAL(0, header.raw[1]);
    TEST_ASSERT_EQUAL(0, header.raw[2]);
    TEST_ASSERT_EQUAL(MDL_ACTION_WRITE, mdl_header_action_get(header));

    mdl_header_action_set(&header, MDL_ACTION_BROADCAST);
    TEST_ASSERT_EQUAL(0b10000000, header.raw[0]);
    TEST_ASSERT_EQUAL(0, header.raw[1]);
    TEST_ASSERT_EQUAL(0, header.raw[2]);
    TEST_ASSERT_EQUAL(MDL_ACTION_BROADCAST, mdl_header_action_get(header));

    mdl_header_action_set(&header, MDL_ACTION_SYNC);
    TEST_ASSERT_EQUAL(0b11000000, header.raw[0]);
    TEST_ASSERT_EQUAL(0, header.raw[1]);
    TEST_ASSERT_EQUAL(0, header.raw[2]);
    TEST_ASSERT_EQUAL(MDL_ACTION_SYNC, mdl_header_action_get(header));
}

//----------------------------------------------------------------------------------------------------------------------

void test_mdl_header_staging_bit_set_get(void)
{
    /* Read error bit is mapped to the same bit as staging bit. */
    mdl_msg_header_t header = {0};

    mdl_header_staging_bit_set(&header, true);
    TEST_ASSERT_EQUAL(0b00100000, header.raw[0]);
    TEST_ASSERT_EQUAL(0, header.raw[1]);
    TEST_ASSERT_EQUAL(0, header.raw[2]);
    TEST_ASSERT_TRUE(mdl_header_staging_bit_get(header));
    TEST_ASSERT_TRUE(mdl_header_read_error_bit_get(header));

    mdl_header_read_error_bit_set(&header, false);
    TEST_ASSERT_EQUAL(0b00000000, header.raw[0]);
    TEST_ASSERT_EQUAL(0, header.raw[1]);
    TEST_ASSERT_EQUAL(0, header.raw[2]);
    TEST_ASSERT_FALSE(mdl_header_staging_bit_get(header));
}

//--------------------------------------------------------------------------------------------------------------------------------------

void test_mdl_header_property_set_get(void)
{
    mdl_msg_header_t header = {0};

    mdl_header_property_set(&header, 0b10); /* This should only affect raw byte [0]. */
    ...
    TEST_ASSERT_EQUAL(0b10, mdl_header_property_get(header));

    mdl_header_property_set(&header, 0b01); /* This should only affect raw byte [1]. */
    TEST_ASSERT_EQUAL(0b00000000, header.raw[0]);
    TEST_ASSERT_EQUAL(0b10000000, header.raw[1]);
    TEST_ASSERT_EQUAL(0b00000000, header.raw[2]);
    TEST_ASSERT_EQUAL(0b01, mdl_header_property_get(header));

    mdl_header_property_set(&header, 0b111111); /* All property bits set. */
    TEST_ASSERT_EQUAL(0b00011111, header.raw[0]);
    TEST_ASSERT_EQUAL(0b10000000, header.raw[1]);
    TEST_ASSERT_EQUAL(0b00000000, header.raw[2]);
    TEST_ASSERT_EQUAL(0b111111, mdl_header_property_get(header));

    memset(header.raw, 0xFF, sizeof(header.raw));
    mdl_header_property_set(&header, 0); /* Clear property */
...
void test_mdl_node_cnt_set_get(void)
{
    mdl_msg_header_t header = {0};

    mdl_header_node_cnt_set(&header, 0b111111); /* This should only affect raw byte [1]*/
    TEST_ASSERT_EQUAL(0b00000000, header.raw[0]);
    TEST_ASSERT_EQUAL(0b00111111, header.raw[1]);
    TEST_ASSERT_EQUAL(0b00000000, header.raw[2]);

    uint16_t node_cnt = mdl_header_node_cnt_get(header);

    TEST_ASSERT_EQUAL(0b111111, node_cnt);
    node_cnt++;
    mdl_header_node_cnt_set(&header, node_cnt); /* This should only affect raw byte [0]*/
    TEST_ASSERT_EQUAL(0b00000000, header.raw[0]);
    TEST_ASSERT_EQUAL(0b00000000, header.raw[1]);
    TEST_ASSERT_EQUAL(0b00000010, header.raw[2]);
    node_cnt = mdl_header_node_cnt_get(header);
    TEST_ASSERT_EQUAL(0b1000000, node_cnt); /* 64 */

    memset(header.raw, 0xFF, sizeof(header.raw));
    mdl_header_node_cnt_set(&header, 0); /* Clear node count */
...
void test_mdl_header_parity_set_get(void)
{
    /* Configure two identical headers. */
    mdl_msg_header_t header_original = {.raw = {0b01100001, 0b00011000, 0b11000000}};
    mdl_msg_header_t header_fixed = {.raw = {0b01100001, 0b00011000, 0b11000000}};

    /* Check that parity is initially wrong. */
    TEST_ASSERT_FALSE(mdl_header_parity_check(header_original));

    /* Fix parity. */
    mdl_header_parity_set(&header_fixed, true);

    /* Check that parity is now correct. */
    TEST_ASSERT_TRUE(mdl_header_parity_check(header_fixed));
    ...
    /* Break parity. */
    mdl_header_parity_set(&header_fixed, false);

    /* Check that parity is now incorrect. */
    TEST_ASSERT_FALSE(mdl_header_parity_check(header_fixed));

    /* Additional test case. */
    header_original = (mdl_msg_header_t){.raw = {0x00, 0x03, 0x01}};
    mdl_header_parity_set(&header_original, true);
    TEST_ASSERT_TRUE(mdl_header_parity_check(header_original)); // Parity should be ok.
}

//----------------------------------------------------------------------------------------------------------------------

void test_mdl_header_sync_type_set_get(void)
{
    mdl_msg_header_t header = {0};

    mdl_header_sync_type_set(&header, MDL_SYNC_ACK);
    TEST_ASSERT_EQUAL(0b00000000, header.raw[0]);
    TEST_ASSERT_EQUAL(0, header.raw[1]);
    TEST_ASSERT_EQUAL(0, header.raw[2]);
    TEST_ASSERT_EQUAL(MDL_SYNC_ACK, mdl_header_sync_type_get(header));

    mdl_header_sync_type_set(&header, MDL_SYNC_COMMIT);
    TEST_ASSERT_EQUAL(0b00010000, header.raw[0]);
    TEST_ASSERT_EQUAL(0, header.raw[1]);
    TEST_ASSERT_EQUAL(0, header.raw[2]);
    TEST_ASSERT_EQUAL(MDL_SYNC_COMMIT, mdl_header_sync_type_get(header));
...
void test_mdl_header_sync_error_set_get(void)
{
    mdl_msg_header_t header = {0};

    header.raw[0] = 0xFF; // Set all bits
    mdl_header_sync_error_set(&header, MDL_SYNC_ERR_NONE);
    TEST_ASSERT_EQUAL(0b11110000, header.raw[0]);
    TEST_ASSERT_EQUAL(0, header.raw[1]);
    TEST_ASSERT_EQUAL(0, header.raw[2]);
    TEST_ASSERT_EQUAL(MDL_SYNC_ERR_NONE, mdl_header_sync_error_get(header));

    mdl_header_sync_error_add(&header, MDL_SYNC_ERR_OTHER);
    TEST_ASSERT_EQUAL(0b11110001, header.raw[0]);
    TEST_ASSERT_EQUAL(0, header.raw[1]);
    TEST_ASSERT_EQUAL(0, header.raw[2]);
    TEST_ASSERT_EQUAL(MDL_SYNC_ERR_OTHER, mdl_header_sync_error_get(header));

    mdl_header_sync_error_add(&header, MDL_SYNC_ERR_TRANSMISSION);
    TEST_ASSERT_EQUAL(0b11110011, header.raw[0]);
    TEST_ASSERT_EQUAL(0, header.raw[1]);
    TEST_ASSERT_EQUAL(0, header.raw[2]);
    TEST_ASSERT_EQUAL(MDL_SYNC_ERR_OTHER | MDL_SYNC_ERR_TRANSMISSION, mdl_header_sync_error_get(header));

    mdl_header_sync_error_add(&header, MDL_SYNC_ERR_HANDLER);
    TEST_ASSERT_EQUAL(0b11110111, header.raw[0]);
    TEST_ASSERT_EQUAL(0, header.raw[1]);
    TEST_ASSERT_EQUAL(0, header.raw[2]);
    TEST_ASSERT_EQUAL(MDL_SYNC_ERR_OTHER | MDL_SYNC_ERR_TRANSMISSION | MDL_SYNC_ERR_HANDLER,
                      mdl_header_sync_error_get(header));

    mdl_header_sync_error_add(&header, MDL_SYNC_ERR_NOT_SUPPORTED);
    TEST_ASSERT_EQUAL(0b11111111, header.raw[0]);
    TEST_ASSERT_EQUAL(0, header.raw[1]);
    TEST_ASSERT_EQUAL(0, header.raw[2]);
    TEST_ASSERT_EQUAL(MDL_SYNC_ERR_OTHER | MDL_SYNC_ERR_TRANSMISSION | MDL_SYNC_ERR_HANDLER | MDL_SYNC_ERR_NOT_SUPPORTED,
                      mdl_header_sync_error_get(header));
}

//----------------------------------------------------------------------------------------------------------------------

void test_mdl_master_prop_read(void)
{
    /* Initialize a test master. */
    mdl_test_master_ctx_t test_master_ctx;
    mdl_test_master_init(&test_master_ctx);
    ...
    expected_data[sizeof(expected_data) - 1] = mdl_checksum_calculate(expected_data, sizeof(expected_data) - 1);
    ...
    mdl_msg_header_t header = {0};
    mdl_header_action_set(&header, MDL_ACTION_READ);
    mdl_header_property_set(&header, PROP_STATIC_RW);
    mdl_header_node_cnt_set(&header, 1);
    mdl_header_parity_set(&header, true);

    tx_payload[0] = header.raw[0];
    tx_payload[1] = header.raw[1];
    tx_payload[2] = header.raw[2];
    mdl_payload_cobs_encode(tx_payload + 3, &tx_payload_len, expected_data, sizeof(expected_data));

    uart_write(&test_node, tx_payload, tx_payload_len + 3);

    /* Try a read all command */
    mdl_master_prop_read(&test_master_ctx.master_ctx, PROP_STATIC_RW);
...
void test_master_read_nodes(void)
{
    ...
    /* Initialize master. */
    mdl_test_master_ctx_t test_master_ctx = {0};
    mdl_test_master_init(&test_master_ctx);
    ...
    mdl_test_node_group_ctx_t node_test_grp = {0};
    setup_mdl_node_prop_list_handlers();
    mdl_test_node_init(&node_test_grp, node_cnt, &test_master_ctx);
    ...
    err = mdl_master_prop_read(&test_master_ctx.master_ctx, PROP_STATIC_RW);
    usleep(10000);
    TEST_ASSERT_EQUAL(MDL_MASTER_OK, err);
    ...
    for (int i = 0; i < node_cnt; i++)
    {
        ...
        TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_test_grp.node_list[i].node_ctx.state);
    }

    /* Deinitialize master and nodes. */
    mdl_test_master_deinit(&test_master_ctx);
    mdl_test_node_deinit(&node_test_grp);
}
...
void test_master_write_nodes(void)
{
    ...
    err = mdl_master_prop_write(&test_master_ctx.master_ctx, PROP_STATIC_RW, 0, false, true);
    usleep(10000);
    TEST_ASSERT_EQUAL(MDL_MASTER_OK, err);
...
{
    mdl_node_ctx_t node_ctx = {0};

    mdl_node_uart_cb_cfg_t uart_cb = (mdl_node_uart_cb_cfg_t){
    ...
    mdl_prop_t *prop_list = (mdl_prop_t *)0xfeedface; // Dummy pointer
    void *prop_userdata = (void *)0xdefec8ed;         // Dummy pointer
    size_t prop_list_size = 10;

    mdl_node_init(&node_ctx, &uart_cb, uart_userdata, prop_list, prop_list_size, prop_userdata);
    ...
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_UNDEFINED, node_ctx.next_state);
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_NONE, node_ctx.last_error);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_UNDEFINED, node_ctx.error_state);
...
{
    /* Test Params: */
    mdl_action_t test_action = MDL_ACTION_READ;
    mdl_prop_id_t test_prop_id = 0; /* Only one handler so must be zero!*/
    ...
    /* Setup the state. */
    node_ctx.state = MDL_NODE_STATE_RX_HEADER;
    node_ctx.next_state = MDL_NODE_STATE_UNDEFINED;

    /* Run the madelink node. */
    mdl_node_tick(&node_ctx, 0); /* Initial Tick. */

    /* Write a dummy header. */
    mdl_msg_header_t header = {0};
    mdl_header_action_set(&header, test_action);
    mdl_header_property_set(&header, test_prop_id);
    mdl_header_node_cnt_set(&header, test_node_cnt);
    mdl_header_parity_set(&header, true);
    ...
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_DEC_NODE_CNT, node_ctx.next_state);

    memcpy(header.raw, uart_ctx.tx_buf, 3); /* Compare the transmitted header. */
    TEST_ASSERT_EQUAL(test_action, mdl_header_action_get(header));
    TEST_ASSERT_EQUAL(test_prop_id, mdl_header_property_get(header));
    TEST_ASSERT_EQUAL(test_node_cnt + 1, mdl_header_node_cnt_get(header));
    TEST_ASSERT_EQUAL(test_node_cnt + 1, node_ctx.node_cnt);
    TEST_ASSERT_TRUE(mdl_header_parity_check(header));
}
...
{
    /* Test Params: */
    mdl_action_t test_action = MDL_ACTION_SYNC;
    mdl_sync_type_t test_sync = MDL_SYNC_ACK;
    mdl_node_err_t expected_error = MDL_SYNC_ERR_OTHER;
    ...
    node_ctx.state = MDL_NODE_STATE_RX_HEADER;
    node_ctx.next_state = MDL_NODE_STATE_UNDEFINED;
    node_ctx.last_error = MDL_NODE_ERR_INVALID_STATE;

    /* Write a dummy header. */
    mdl_msg_header_t header = {0};
    mdl_header_action_set(&header, test_action);
    mdl_header_sync_type_set(&header, test_sync);
    ...
    TEST_ASSERT_EQUAL(test_action, mdl_header_action_get(header));
    TEST_ASSERT_EQUAL(test_sync, mdl_header_sync_type_get(header));
    TEST_ASSERT_EQUAL(expected_error, mdl_header_sync_error_get(header));
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.next_state);
}
...
void test_node_rx_header_state_sync_commit_ok(void)
{
    /* Test Params: */
    mdl_action_t test_action = MDL_ACTION_SYNC;
    mdl_sync_type_t test_sync = MDL_SYNC_COMMIT;
    mdl_node_err_t expected_error = MDL_SYNC_ERR_NONE; /* Error is not used in sync commit. */
    ...
    node_ctx.state = MDL_NODE_STATE_RX_HEADER;
    node_ctx.next_state = MDL_NODE_STATE_UNDEFINED;
    node_ctx.last_error = MDL_NODE_ERR_INVALID_STATE; /* Should not be used. */
    ...
    TEST_ASSERT_EQUAL(expected_error, mdl_header_sync_error_get(header));
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_COMMIT_PROP, node_ctx.next_state);
}
...
void test_node_rx_header_state_read_action_parity_error(void)
{
    /* Test Params: */
    mdl_action_t test_action = MDL_ACTION_READ;
    mdl_prop_id_t test_prop_id = 1; /* Only one handler so must be one!*/
    ...
    mdl_header_property_set(&header, test_prop_id);
    mdl_header_node_cnt_set(&header, test_node_cnt);
    mdl_header_parity_set(&header, false); /* Intentionally set parity wrong. */
    uart_ctx.cnt_readable = 3;
    memcpy(uart_ctx.rx_buf, header.raw, 3);
    ...
    TEST_ASSERT_EQUAL(3, node_ctx.data_cnt);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_ERROR, node_ctx.next_state);
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_HEADER_PARITY, node_ctx.last_error);

    memcpy(header.raw, uart_ctx.tx_buf, 3); /* Compare the transmitted header. */
    TEST_ASSERT_EQUAL(test_action, mdl_header_action_get(header));
    TEST_ASSERT_EQUAL(test_prop_id, mdl_header_property_get(header));
    TEST_ASSERT_EQUAL(test_node_cnt + 1, mdl_header_node_cnt_get(header));
    TEST_ASSERT_FALSE(mdl_header_parity_check(header));
}
...
void test_node_rx_header_state_read_action_timeout_error(void)
{
    ...
    mdl_node_tick(&node_ctx, MDL_NODE_TIMEOUT_MS + 1);
    TEST_ASSERT_EQUAL(1, uart_ctx.tx_cnt);
    TEST_ASSERT_EQUAL(1, node_ctx.data_cnt);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_ERROR, node_ctx.next_state);
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_TIMEOUT, node_ctx.last_error);
}
...
{
    /* Test Params: */
    mdl_action_t test_action = MDL_ACTION_WRITE;
    mdl_prop_id_t test_prop_id = 0; /* Only one handler so must be zero!*/
...
void test_node_rx_header_state_write_action_node_cnt_error(void)
{
    ...
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_ERROR, node_ctx.next_state);
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_INVALID_STATE, node_ctx.last_error);
...
void test_node_dec_node_cnt_state_ok(void)
{
    ...
    node_ctx.state = MDL_NODE_STATE_DEC_NODE_CNT;
    node_ctx.next_state = MDL_NODE_STATE_UNDEFINED;
    node_ctx.property_id = 0; /* Only one handler so must be zero!*/
    node_ctx.staged_write = false;
    node_ctx.action = MDL_ACTION_READ;
    node_ctx.node_cnt = 2;
    mdl_node_tick(&node_ctx, 0);
    TEST_ASSERT_EQUAL(1, node_ctx.node_cnt);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_DEC_NODE_CNT, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_PROP, node_ctx.next_state);
    ...
    TEST_ASSERT_EQUAL(0, node_ctx.node_cnt);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_DEC_NODE_CNT, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_TX_PROP, node_ctx.next_state);
    ...
    node_ctx.property_id = 0; /* Only one handler so must be zero!*/
    node_ctx.staged_write = false;
    node_ctx.action = MDL_ACTION_WRITE;
    node_ctx.node_cnt = 2;
    mdl_node_tick(&node_ctx, 0);
    TEST_ASSERT_EQUAL(1, node_ctx.node_cnt);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_DEC_NODE_CNT, node_ctx.state);
...
void test_node_tx_prop_state_ok(void)
{
    ...
    node_ctx.state = MDL_NODE_STATE_TX_PROP;
    node_ctx.next_state = MDL_NODE_STATE_UNDEFINED;
    node_ctx.property_id = 0; /* Only one handler so must be zero!*/
    ...
    mdl_payload_cobs_decode(decoded_data, &decoded_size, uart_ctx.tx_buf, uart_ctx.tx_cnt);
    TEST_ASSERT_EQUAL(10, decoded_size);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(prop_ctx.data, decoded_data, 10);

    TEST_ASSERT_EQUAL(MDL_NODE_STATE_TX_PROP, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.next_state);
}
...
void test_node_tx_prop_state_read_error_ok(void)
{
    ...
    node_ctx.next_state = MDL_NODE_STATE_UNDEFINED;
    node_ctx.property_id = 0;                              /* Only one handler so must be zero!*/
    mdl_header_read_error_bit_set(&node_ctx.header, true); /* Set read_error bit. */
    node_ctx.last_error = MDL_NODE_ERR_CHECKSUM;           /* Set error for test.. */
    node_ctx.error_state = MDL_NODE_STATE_TX_PROP;         /* Set error state for test.*/
    ...
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_CHECKSUM, decoded_data[0]);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_TX_PROP, decoded_data[1]);
...
void test_node_tx_prop_state_timeout_error(void)
{
    ...
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_TX_PROP, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_ERROR, node_ctx.next_state);
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_TIMEOUT, node_ctx.last_error); /* Error state */
}
...
void test_node_tx_prop_state_error(void)
{
    ...
    node_ctx.state = MDL_NODE_STATE_TX_PROP;
    node_ctx.next_state = MDL_NODE_STATE_TX_PROP;
    ...
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_TX_PROP, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.next_state);
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_PROP_NOT_SUPPORTED, node_ctx.last_error); /* Error state */

    /* Case 2 : No get handler. */
    mdl_prop_handler_cb_t old_handler = node_ctx.prop_list[0].handler.get;
    node_ctx.state = MDL_NODE_STATE_TX_PROP;
    node_ctx.next_state = MDL_NODE_STATE_TX_PROP;
    ...
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_TX_PROP, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.next_state);
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_READ_NOT_SUPPORTED, node_ctx.last_error); /* Error state */
    ...
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_TX_PROP, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.next_state);
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_READ_CB, node_ctx.last_error); /* Error state */
    node_ctx.prop_list[0].handler.get = old_handler;              /* Restore get handler */
    ...
    uart_ctx.tx_buf[0] = 0xFF;                 /* Fill TX buffer with dummy data to detect if anything is sent. */
    uart_ctx.tx_cnt = 0;                       /* Reset TX counter */
    prop_ctx.size = MDL_PROPERTY_SIZE_MAX + 1; /* Set property size larger than max payload size. */
    prop_ctx.success = true;
    mdl_node_tick(&node_ctx, 0);
    TEST_ASSERT_EQUAL(0x00, uart_ctx.tx_buf[0]); /* Only one byte with value 0x00 should be sent. */
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_TX_PROP, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.next_state);
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_COBS_ENC, node_ctx.last_error); /* Error state */
}
...
void test_node_rx_prop_state_no_forwarding_ok(void)
{
    ...
    mdl_node_ctx_t node_ctx = {0};
    node_dummy_config(&node_ctx);
    node_ctx.state = MDL_NODE_STATE_RX_PROP;
    node_ctx.next_state = MDL_NODE_STATE_UNDEFINED;
    node_ctx.property_id = 0;           /* Only one handler so must be zero!*/
    node_ctx.action = MDL_ACTION_WRITE; /* No forwarding */
    ...
    test_data[sizeof(test_data) - 1] = mdl_checksum_calculate(test_data, sizeof(test_data) - 1);
    uart_ctx.cnt_readable = 1000;
    mdl_payload_cobs_encode(uart_ctx.rx_buf, &uart_ctx.cnt_readable, test_data, sizeof(test_data));
    uart_ctx.cnt_writable = 1000;

    mdl_node_tick(&node_ctx, 0);

    TEST_ASSERT_EQUAL(0, uart_ctx.tx_cnt); /* Check that no bytes were sent. */
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_PROP, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_DEC_NODE_CNT, node_ctx.next_state);
}
...
void test_node_rx_prop_state_forwarding_ok(void)
{
    ...
    node_ctx.next_state = MDL_NODE_STATE_RX_PROP;
    node_ctx.action = MDL_ACTION_BROADCAST; /* Forwarding */
    node_ctx.node_cnt = 0;                  /* No forwarding */
    uart_ctx.rx_cnt = 0;
    uart_ctx.tx_cnt = 0;

    uart_ctx.cnt_writable = 7;
    mdl_node_tick(&node_ctx, 0); /* Write 7 bytes in first go. */
    TEST_ASSERT_EQUAL(7, uart_ctx.tx_cnt);
    uart_ctx.cnt_writable = 7;
    mdl_node_tick(&node_ctx, 0); /* Write 6 more bytes in second go. */
...
void test_node_rx_prop_state_error(void)
{
    ...
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_PROP, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_ERROR, node_ctx.next_state);
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_COBS_DEC, node_ctx.last_error); /* Error state */
    ...
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_PROP, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_ERROR, node_ctx.next_state);
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_CHECKSUM, node_ctx.last_error); /* Error state */
...
void test_node_commit_prop_state_ok(void)
{
    ...
    mdl_node_ctx_t node_ctx = {0};
    node_dummy_config(&node_ctx);
    node_ctx.next_state = MDL_NODE_STATE_COMMIT_PROP;
    ...
    mdl_node_tick(&node_ctx, 0);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_COMMIT_PROP, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_UNDEFINED, node_ctx.next_state);
...
void test_node_commit_prop_state_fail(void)
{
    ...
    node_ctx.property_id = 0; /* Only one handler so must be zero!*/
    prop_ctx.success = true;
    mdl_prop_handler_cb_t original_set_handler = node_ctx.prop_list[0].handler.set;
    node_ctx.prop_list[0].handler.set = NULL; /* Remove set handler */

    mdl_node_tick(&node_ctx, 0);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_COMMIT_PROP, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_ERROR, node_ctx.next_state);
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_WRITE_NOT_SUPPORTED, node_ctx.last_error); /* Error state */
    ...
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_COMMIT_PROP, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_ERROR, node_ctx.next_state);
    TEST_ASSERT_EQUAL(MDL_NODE_ERR_WRITE_CB, node_ctx.last_error); /* Error state */
}
...
void test_node_error_state_ok(void)
{
    ...
    /* No Data */
    node_ctx.next_state = MDL_NODE_STATE_ERROR;
    mdl_node_tick(&node_ctx, 0);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_ERROR, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_UNDEFINED, node_ctx.next_state);
    ...
    mdl_node_tick(&node_ctx, 0);
    TEST_ASSERT_EQUAL(0, uart_ctx.tx_cnt); /* Ensure no data is transmitted */
    ...
    /* Timer Elapses */
    uart_ctx.tx_busy = false;
    mdl_node_tick(&node_ctx, 3 * (MDL_NODE_TIMEOUT_MS + 1));
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_ERROR, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_RX_HEADER, node_ctx.next_state);
}
...
void test_mdl_node_is_busy_ok(void)
{
    /* Setup a node. */
    mdl_node_ctx_t node_ctx = {0};
    node_dummy_config(&node_ctx);
    ...
    node_ctx.state = MDL_NODE_STATE_RX_HEADER;
    uart_ctx.tx_busy = false;
    TEST_ASSERT_FALSE(mdl_node_is_busy(&node_ctx));

    /* Busy */
    node_ctx.state = MDL_NODE_STATE_RX_HEADER;
    uart_ctx.tx_busy = true;
    TEST_ASSERT_TRUE(mdl_node_is_busy(&node_ctx));
...
void test_mdl_node_tick_invalid_state(void)
{
    /* Setup a node. */
    mdl_node_ctx_t node_ctx = {0};
    node_dummy_config(&node_ctx);

    node_ctx.state = MDL_NODE_STATE_UNDEFINED;
    node_ctx.next_state = MDL_NODE_STATE_UNDEFINED;

    mdl_node_tick(&node_ctx, 0);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_UNDEFINED, node_ctx.state);
    TEST_ASSERT_EQUAL(MDL_NODE_STATE_UNDEFINED, node_ctx.next_state);
}
...
int main(void)
{
    ...
    RUN_TEST(test_mdl_cobs_encode_decode_ok);
    RUN_TEST(test_mdl_cobs_encode_nok);
    RUN_TEST(test_mdl_cobs_decode_nok);
    RUN_TEST(test_mdl_checksum_calculate);
    RUN_TEST(test_mdl_parity_check);
    RUN_TEST(test_mdl_node_cnt_set_get);
    RUN_TEST(test_mdl_header_action_set_get);
    RUN_TEST(test_mdl_header_staging_bit_set_get);
    RUN_TEST(test_mdl_header_property_set_get);
    RUN_TEST(test_mdl_header_parity_set_get);
    RUN_TEST(test_mdl_header_sync_type_set_get);
    RUN_TEST(test_mdl_header_sync_error_set_get);

    /* Test Master functions. */
    // RUN_TEST(test_mdl_master_prop_read);
    ...
    RUN_TEST(test_mdl_node_is_busy_ok);
    RUN_TEST(test_mdl_node_tick_invalid_state);
...
void node_dummy_config(mdl_node_ctx_t *node_ctx)
{
    ...
    memset(node_ctx, 0, sizeof(mdl_node_ctx_t));
