...
// static void *mdl_test_node_thread_loop(void *arg);
static void *mdl_test_node_thread_loop(void *arg);
...
{
    const mdl_node_uart_cb_cfg_t uart_cb = {
    ...
    node_test_grp->node_list = malloc(node_cnt * sizeof(mdl_test_node_ctx_t));
    if (!node_test_grp->node_list)
    {
        perror("malloc");
        exit(EXIT_FAILURE);
    }
    memset(node_test_grp->node_list, 0, node_cnt * sizeof(mdl_test_node_ctx_t));
    for (size_t i = 0; i < node_cnt; i++)
    {
        ...
        mdl_node_init(&node_test_grp->node_list[i].node_ctx, &uart_cb, &node_test_grp->node_list[i].uart, mdl_prop_list,
    ...
    if (pthread_create(&node_test_grp->thread, NULL, mdl_test_node_thread_loop, node_test_grp) != 0)
    {
...
{
    mdl_prop_list[PROP_STATIC_RW].handler.get = node_get_handler;
    mdl_prop_list[PROP_STATIC_RW].handler.set = node_set_handler;

    mdl_prop_list[PROP_STATIC_RO].handler.get = node_get_handler;
    mdl_prop_list[PROP_STATIC_RO].handler.set = NULL;

    mdl_prop_list[PROP_STATIC_WO].handler.get = NULL;
    mdl_prop_list[PROP_STATIC_WO].handler.set = node_set_handler;

    mdl_prop_list[PROP_DYNAMIC_RW].handler.get = node_get_handler;
    mdl_prop_list[PROP_DYNAMIC_RW].handler.set = node_set_handler;

    mdl_prop_list[PROP_DYNAMIC_RO].handler.get = node_get_handler;
    mdl_prop_list[PROP_DYNAMIC_RO].handler.set = NULL;

    mdl_prop_list[PROP_DYNAMIC_WO].handler.get = NULL;
    mdl_prop_list[PROP_DYNAMIC_WO].handler.set = node_set_handler;
}

static void *mdl_test_node_thread_loop(void *arg)
{
    mdl_test_node_group_ctx_t *node_test_grp = (mdl_test_node_group_ctx_t *)arg;
    ...
    while (node_test_grp->running)
    {
        ...
        mdl_node_tick(&node_test_grp->node_list[ctx_idx].node_ctx, ms);
...
{
    (void)node_idx;
    mdl_test_node_ctx_t *ctx = (mdl_test_node_ctx_t *)userdata;
